...
où $B$ est une base de \(A\) (matrice extraite de \(A\) telle que \(\det(B)\neq
0\)), $I(B)$ (resp. $J(B)$) sont les indices des colonnes de $A$ inclus (resp.
exclus) dans $B$. Passer à un sommet adjacent revient à effectuer un
\textbf{changement de base} en y ajoutant un indice exclus $k$ et en retirant un
indice inclus $r$, de manière à obtenir :
\[\begin{dcases}
...
\section{Dualité}
\begin{definition}
Soit le problème linéaire en forme standard, qui est appelé \emph{problème primal}
\[\begin{cases}
\text{Max } z=cx \\
Ax=b \\
x \ge 0
...
A^Ty\ge c^T
\end{cases}\]
avec \(y=[y_1,\dots,y_m]\in\mathbb{R}^m\)
étant les nouvelles variables du dual. Notez que ces variables ne sont pas
restreintes aux rééls non-négatifs.
\end{definition}
\end{document}
