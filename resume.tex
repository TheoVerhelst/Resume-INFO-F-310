...
admissibles \(P = \{x:Ax=b,x\ge 0\}\) est une solution de base admissible.
\begin{proof}
Soit \(s=(s_1,\dots,s_k,0\dots,0)\) un sommet de \(P\) (c'est donc une solution
admissible), avec \(s_i\ge0\;\forall i\in[1,k]\). On peut toujours s'y ramener
...
\alpha_j|< s_j \;\forall j\in[1,k]\), on peut écrire:
\[\begin{dcases}
...
\begin{note}
On a imposé la condition \(|\epsilon \alpha_j|< s_j \;\forall j\in[1,k]\) afin
