...
où $B$ est une base de \(A\) (matrice de rang \(m\) extraite de \(A\)), $I(B)$
(resp. $J(B)$) sont les indices des colonnes de $A$ inclus (resp. exclus) dans
$B$. Passer à un sommet adjacent revient à effectuer un \textbf{changement de
base} en y ajoutant un indice exclus $k$ et en retirant un indice inclus $r$, de
manière à obtenir :
\[\begin{dcases}
